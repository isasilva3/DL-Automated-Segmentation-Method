...
data_dir = os.path.join(root_dir, "Lungs_Data")
...
train_loader = DataLoader(train_ds, batch_size=1, shuffle=False, num_workers=0) #Shuffle false
...
for epoch in range(epoch_num):
    ...
    for batch_data in train_loader:
        ...
        image, label = (inputs[0][0].cpu().numpy(), labels[0][0].cpu().numpy())
        fig = plt.figure("check", (12, 12))
        plt.subplot(1, 2, 1)
        plt.title("image")
